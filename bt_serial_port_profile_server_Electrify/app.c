...
#define SPP_OPERATION_MODE SPP_SERVER_MODE  // Define mode as a server

#define DEVICE_NAME_LENGTH 10

char DEVICE_NAME[DEVICE_NAME_LENGTH][DEVICE_NAME_LENGTH + 1];

char deviceNameLength =10;
int i;
 ...
 *   throughput */
#define UART_POLL_TIMEOUT  5000   //changes done to received the large packet
...
static uint8_t advertising_set_handle = 0xff;  // server is in advertising mode
...
static uint8_t max_packet_size = 256;
...
void updateDeviceNameGATT() {


  size_t deviceNameLength = 10;

  result = sl_bt_gatt_server_write_attribute_value(gattdb_SPP_Server, 0, deviceNameLength, (uint8_t*)DEVICE_NAME);

  if (result == SL_STATUS_OK) {
      app_log("Write device name to GATT database succeeded\r\n");
  } else {
      app_log("Failed to write device name to GATT database, error code: 0x%04X\r\n", result);
  }


          app_log("DEVICE_NAME[%d]: %s\n", i, DEVICE_NAME[i]);

          // Stop advertising and then start it again to make the new device name visible.
...
void updateDeviceName(uint8_t i, const uint8_t* data, uint32_t length) {
      if (length >= 11 && memcmp(data, "dnc", 3) == 0) {
          // Copy the next 10 bytes to DEVICE_NAME[i]
          memcpy(DEVICE_NAME[i], data + 3, DEVICE_NAME_LENGTH);
          // Null-terminate the string
          DEVICE_NAME[i][DEVICE_NAME_LENGTH] = '\0';

          updateDeviceNameGATT();
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:    // this case is advertising mode    app_log("SPP Role: SPP Server\r\n");
      // reset_variables();
    ...
    case sl_bt_evt_connection_opened_id:  // client connected state
...
static void send_spp_data()
{
  ...
  updateDeviceName(i, data, sizeof(data));
